...
def get_recipe(recipe_id):
    if recipe_id in ia_recipes_cache:
        return jsonify(ia_recipes_cache[recipe_id])
    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True)
    cursor.execute(
...
def generate_ai_recipe():
    ...
    prompt = (
        ...
        "Cada receta debe tener los campos: id (comenzando desde el número 1000),description (con una frase corta), title, ingredients (objeto con el nombre del ingrediente como key y la cantidad como value), "
        ...
        f"{ingredients}. Si el usuario indica algo que no sean ingredientes. Devuelve un string que indique 'invalid'"
    )
...
# endpoint used to store in cache the recipe generated by ia then send it to frontend
@recipes_bp.route("/api/recipes/save_ai_recipe", methods=["POST"])
@jwt_required()
def save_ai_recipe():
    data = request.get_json()
    recipe_id = data.get("id")
    if not recipe_id:
        return jsonify({"error": "Falta la id de la receta!"}), 400
    ia_recipes_cache[recipe_id] = data
    return jsonify({"message": "Receta IA guardada correctamente", "id": recipe_id})
